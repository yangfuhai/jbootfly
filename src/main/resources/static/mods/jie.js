...
layui.define('fly', function(exports){
  ...
  //提交回答
  fly.form['/api/commentAdd'] = function(data, required){
    var tpl = '<li>\
  ...
  gather.jieAdmin = {
    ...
    //设置置顶、状态
    ,setLevel: function(div){
      var othis = $(this);
      fly.json('/api/postLevel', {
        id: div.data('id')
        ,level: othis.attr('level')
      }, function(res){
        if(res.state === "ok"){
          location.reload();
        }
    ...
    ,recommend: function(div){
      var othis = $(this);
      fly.json('/api/postRecommend', {
        id: div.data('id')
      }, function(res){
        if(res.state === "ok"){
          location.reload();
    ...
    ,recommendCancel: function(div){
      var othis = $(this);
      fly.json('/api/postRecommendCancel', {
        id: div.data('id')
      }, function(res){
        if(res.state === "ok"){
          location.reload();
    ...
    ,collect: function(div){
      ...
      fly.json('/api/collection'+ type +'/', {
        id: div.data('id')
      }, function(res){
        if(type === 'add'){
          othis.data('type', 'Del').html('取消收藏').addClass('layui-btn-danger');
        } else if(type === 'remove'){
  ...
  var asyncRender = function(){
    ...
    if(jieAdmin[0] && layui.cache.user.uid != -1){
      fly.json('/api/collectionFind', {
        id: div.data('id')
      }, function(res){
        jieAdmin.append('<span class="layui-btn layui-btn-xs jie-admin '+ (res.collection ? 'layui-btn-danger' : '') +'" type="collect" data-type="'+ (res.collection ? 'Del' : 'Add') +'">'+ (res.collection ? '取消收藏' : '收藏') +'</span>');
      });
    }
  }();
